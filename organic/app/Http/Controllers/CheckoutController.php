...
class CheckoutController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (auth()->check()) {
            ...
            $all_carts_count = Cart::where('user_id', $user_id)->count();
            ...
            $all_carts_count = ($temp_user_id != null) ? Cart::where('temp_user_id', $temp_user_id)->count() : 0;
        }
    ...
    {
        // Check if guest user exists by email
        if (auth()->user() == null) {
        ...
        foreach ($carts as $cartItem) {
            $product = Product::find($cartItem['product_id']);
            if (!$product || !$product->isApprovedPublished()) {
                flash(translate('One or more products are not available.'))->error();
                return redirect()->route('cart');
            }

            $product_stock = $product->stocks->where('variant', $cartItem['variation'])->first();
            if (!$product_stock) {
                // Try to find stock with empty variant for non-variant products
                $product_stock = $product->stocks->filter(function($stock) {
                    return $stock->variant == '' || $stock->variant == null;
                })->first();
            }

            if (!$product_stock || $product_stock->qty < $cartItem['quantity']) {
                flash(translate('Insufficient stock for ' . $product->getTranslation('name')))->error();
                return redirect()->route('cart');
            }
        }
        ...
        try {
            ...
            flash(translate('An error occurred while placing your order. Please try again.'))->error();
            return redirect()->route('checkout');
        }
    ...
    public function store_delivery_info(Request $request)
    {
        ...
        if ($carts && count($carts) > 0) {
            foreach ($carts as $key => $cartItem) {
                ...
                $shipping += round(session('shipping_cost', 0) / count($carts));
                $cartItem->save();
            }
            $shipping = round($shipping);
            $total = $subtotal + $tax + $shipping;
                ...
                if (($user == null && Session::has('temp_user_id')) || CouponUsage::where('user_id', $user->id)->where('coupon_id', $coupon->id)->first() == null) {
                    ...
                    if ($coupon->type == 'cart_base' || $coupon->type == 'welcome_base') {
                        ...
                        foreach ($user_carts as $key => $cartItem) {
                            ...
                            $subtotal += cart_product_price($cartItem, $product, false, false) * $cartItem['quantity'];
                            $tax += cart_product_tax($cartItem, $product, false) * $cartItem['quantity'];
                            $shipping += session('shipping_cost', 0) / count($cartItem);
                        }
    ...
    public function updateDeliveryInfo(Request $request)
    {
        ...
        return view('frontend.partials.cart.cart_summary_2', compact('carts', 'proceed'))->render();
    }
