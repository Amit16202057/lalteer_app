...
class OrderController extends Controller
{
    public function store(Request $request, $set_paid = false)
    {
        ...
        $combined_order->grand_total = 0; // Initialize grand_total to 0
        $combined_order->save();
        ...
        foreach ($seller_products as $seller_product) {
            ...
            foreach ($seller_product as $cartItem) {
                ...
                // Get shipping cost from cart item (API doesn't use session)
                // Cart items can be accessed as both object and array in Laravel
                $cartItemShippingCost = 0;
                if (is_object($cartItem) && isset($cartItem->shipping_cost)) {
                    $cartItemShippingCost = $cartItem->shipping_cost;
                } elseif (isset($cartItem['shipping_cost'])) {
                    $cartItemShippingCost = $cartItem['shipping_cost'];
                }
                
                // If shipping cost is not set in cart item, try to get from request or calculate proportionally
                if ($cartItemShippingCost <= 0) {
                    // Check if shipping_cost is in request (some APIs might send it)
                    if ($request->has('shipping_cost') && $request->shipping_cost > 0) {
                        // Divide total shipping cost by number of items in current seller's product array
                        $cartItemShippingCost = $request->shipping_cost / count($seller_product);
                    } else {
                        // Try to calculate from all cart items
                        $totalShippingFromCart = $cartItems->sum(function($item) {
                            if (is_object($item) && isset($item->shipping_cost)) {
                                return $item->shipping_cost;
                            } elseif (isset($item['shipping_cost'])) {
                                return $item['shipping_cost'];
                            }
                            return 0;
                        });
                        if ($totalShippingFromCart > 0 && count($cartItems) > 0) {
                            $cartItemShippingCost = $totalShippingFromCart / count($cartItems);
                        }
                    }
                }
                $order_detail->shipping_cost = $cartItemShippingCost;
