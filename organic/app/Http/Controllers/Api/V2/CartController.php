...
{
    /**
     * Mobile app can end up with all items status=0 from older flows.
     * If there are no active items but carts exist, restore them as active.
     */
    private function restoreAllInactiveCarts(?int $user_id, ?string $temp_user_id): void
    {
        if ($user_id !== null) {
            $hasActive = Cart::where('user_id', $user_id)->active()->exists();
            if (!$hasActive && Cart::where('user_id', $user_id)->exists()) {
        ...
        if ($temp_user_id !== null) {
            $hasActive = Cart::where('temp_user_id', $temp_user_id)->active()->exists();
            if (!$hasActive && Cart::where('temp_user_id', $temp_user_id)->exists()) {
                Cart::where('temp_user_id', $temp_user_id)->update(['status' => 1]);
            }
        }
    }

    /**
     * Resolve cart owner: prefer authenticated user from Bearer token,
     * then request user_id, then request temp_user_id. Empty strings treated as null.
     */
    private function resolveCartIdentifiers(Request $request): array
    {
        $user_id = null;
        $temp_user_id = null;

        // If Bearer token present, use that user so cart works even when app sends wrong/empty body
        ...
        if ($user_id === null) {
            ...
            // Treat empty string and "0" as null (app may send 0 before user_id is loaded)
            $user_id = ($uidStr !== '' && $uidStr !== '0') ? $uid : null;
        }
        $tid = $request->input('temp_user_id');
        $tidStr = trim((string) ($tid ?? ''));
        $temp_user_id = $tidStr !== '' ? $tid : null;

        return ['user_id' => $user_id, 'temp_user_id' => $temp_user_id];
    }

    public function summary(Request $request)
    {
        ['user_id' => $user_id, 'temp_user_id' => $temp_user_id] = $this->resolveCartIdentifiers($request);
        $this->restoreAllInactiveCarts($user_id, $temp_user_id);
        $user = ($user_id !== null) ? User::where('id', $user_id)->first() : null;
        $items = ($user != null) ?
                Cart::where('user_id', $user->id)->active()->get() :
                ($temp_user_id !== null ? Cart::where('temp_user_id', $temp_user_id)->active()->get() : collect());
                ...
                if (!empty($shop_items_raw_data)) {
                    foreach ($shop_items_raw_data as $shop_items_raw_data_item) {
                        ...
                        $stockRow = $product->stocks
                            ->where('variant', $shop_items_raw_data_item['variation'])
                            ->first();
                        $upperLimit = $stockRow ? (int) $stockRow->qty : 0;
                        ...
                        $shop_items_data_item["upper_limit"] = $upperLimit;
