...
if (!function_exists('normalize_variant_value')) {
    function normalize_variant_value($variant)
    {
        if ($variant === null) {
            return '';
        }

        return mb_strtolower(preg_replace('/\s+/', '', trim((string) $variant)));
    }
}

if (!function_exists('find_product_stock_by_variant')) {
    function find_product_stock_by_variant($product, $variant = null, $allowDefaultFallback = true)
    {
        if (!$product || !$product->relationLoaded('stocks') && !method_exists($product, 'stocks')) {
            return null;
        }
        ...
        $variant = $variant ?? '';
        $normalizedVariant = normalize_variant_value($variant);

        if ($normalizedVariant !== '') {
            $exactStock = $stocks->where('variant', $variant)->first();
            if ($exactStock) {
                return $exactStock;
            }

            return $stocks->first(function ($stock) use ($normalizedVariant) {
                return normalize_variant_value($stock->variant) === $normalizedVariant;
            });
        }

        $emptyVariantStock = $stocks->first(function ($stock) {
            return $stock->variant === '' || $stock->variant === null;
        });

        if ($emptyVariantStock) {
            return $emptyVariantStock;
        }

        return $allowDefaultFallback ? $stocks->first() : null;
    }
}
    ...
    function cart_product_price($cart_product, $product, $formatted = true, $tax = true)
    {
        if ($product->auction_product == 0) {
            ...
            if ($product_stock == null) {
                $product_stock = $product->stocks->filter(function ($stock) {
                    return $stock->variant == '' || $stock->variant == null;
                })->first();
            }
            if ($product_stock == null) {
            ...
            $price = $product_stock ? $product_stock->price : ($product->unit_price ?? 0);
...
if (!function_exists('home_price_cart')) {
    function home_price_cart($product, $formatted = true, $variant = null)
    {
        $price = $product->unit_price;
        if ($product->variant_product && $variant) {
            $stock = find_product_stock_by_variant($product, $variant, false);
            if ($stock) {
                $price = $stock->price;
            }
        }
        ...
        foreach ($product->taxes as $product_tax) {
            if ($product_tax->tax_type == 'percent') {
                $price += ($price * $product_tax->tax) / 100;
            } elseif ($product_tax->tax_type == 'amount') {
                $price += $product_tax->tax;
            }
        }

        // Return formatted or raw price
        if ($formatted) {
            return format_price(convert_price($price));
        } else {
            return $price;
...
if (!function_exists('home_discounted_price_cart')) {
    function home_discounted_price_cart($product, $variant = null, $formatted = true)
    {
        $price = $product->unit_price;
...
if (!function_exists('get_single_product')) {
    ...
    {
        $product_query = Product::query()->with('thumbnail', 'reviews');
