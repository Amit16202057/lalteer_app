...
<div class="aiz-card-box h-auto bg-white py-3 hov-scale-img box_padding ">
    <div class="position-relative img-fit overflow-hidden border">
        ...
        @if ($cardDiscountPercent > 0)
            ...
            style="padding-top:2px;padding-bottom:2px;">-{{ $cardDiscountPercent }}%</span>
        @endif
        <!-- Wholesale tag -->
        @if ($product->wholesale_product)
            <span class="absolute-top-left fs-11 text-white fw-700 px-2 lh-1-8 ml-1 mt-1"
                style="background-color: #455a64; @if ($cardDiscountPercent > 0) top:25px; @endif">
                {{ translate('Wholesale') }}
            </span>
        @endif
        ...
        <div class="product-overlay">
            <div class="overlay-content">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="text-white fs-16 fw-700 text-right">{{ $displayDiscountedPrice }}
                        @if ($displayBasePrice != $displayDiscountedPrice)
                            <sup> <del class="fw-400 text-secondary mr-1">{{ $displayBasePrice }}</del></sup>
                        @endif
                    </div>
                ...
                <div class="buy_now d-flex justify-content-center">
                    <button class="btn btn-primary" onclick="showAddToCartModal({{ $product->id }})">{{ translate('Buy Now') }}</button>
                </div>
            </div>
        </div>
    ...
    <div class="pro_size d-flex justify-content-center align-items-center">
        <h5 class="mb-0 text-dark pr-2 product-box-size-responsive">{{ translate('Size : ') }}</h5>

        <div class="size_btn product-box-size-responsive-btn" onclick="showAddToCartModal({{ $product->id }})">
            {{ translate($firstVariation) }}<span><img width="30px"
