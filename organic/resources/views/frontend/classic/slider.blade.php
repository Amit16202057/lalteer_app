                        ...
                        <div class="carousel-box position-relative">
                            ...
                            <div class="slider-overlay">
                                <h2 class="slider-heading">
                                    {{ isset(json_decode($home_slider_title1, true)[$key]) ? json_decode($home_slider_title1, true)[$key] : '' }}
                                </h2>
                                <p class="slider-description">
                                    {{ isset(json_decode($home_slider_description, true)[$key]) ? json_decode($home_slider_description, true)[$key] : '' }}
                                </p>
                                <div class="slider-buttons">
                                    <a href="{{ isset(json_decode($home_slider_btn1, true)[$key]) ? json_decode($home_slider_btn1, true)[$key] : '' }}"
                                        class="btn btn-primary">{{ translate('Shop Now') }}</a>
                                    <a href="{{ isset(json_decode($home_slider_btn2, true)[$key]) ? json_decode($home_slider_btn2, true)[$key] : '' }}"
                                        class="btn btn-light text-dark ml-2">{{ translate('Contact Us') }}</a>
                                </div>
                            </div>
                        </div>
                    @endforeach
...
@endphp
<div style="background-image: url('{{ $firstValue ? uploaded_asset($firstValue) : asset('default-placeholder-image.jpg') }}');"
    ...
    <div class="container below_slider">
        @foreach ($sliders as $key => $slider)
            <div class="item_1">
                <img height="60px" width="60px" src="{{ $slider ? my_asset($slider->file_name) : static_asset('assets/img/placeholder.jpg') }}"
                    alt="">
                <div class="ex_shipping">
                    {{ isset(json_decode($exclusive_shipping_title1, true)[$key]) ? json_decode($exclusive_shipping_title1, true)[$key] : '' }}
                </div>
                <div class="ex_description">
                    {{ isset(json_decode($exclusive_shipping_description, true)[$key]) ? json_decode($exclusive_shipping_description, true)[$key] : '' }}
                </div>
            </div>
        @endforeach
    </div>
    ...
    <div class="exclusive_btn">
        ...
        <p>{{ translate('Download Catalogue') }}</p>
        </div>
    <div class="dropdown-menu" id="dropdownMenu">
            @if (is_array($titles) && count($titles) > 0)
                @foreach ($titles as $index => $title)
                    @php $link = $links[$index] ?? '#'; @endphp
                    <a href="{{ uploaded_asset($link) }}" class="dropdown-item" data-file="{{ uploaded_asset($link) }}">
                        {{ $title }}
                    </a>
                @endforeach
            @endif
            ...
            <div>
                <button id="downloadBtn" class="btn btn-primary exclusive_btn-download">{{ translate('Download') }}</button>
                <button id="previewBtn" class="btn btn-secondary exclusive_btn-download">{{ translate('Preview') }}</button>
            </div>
    </div>
