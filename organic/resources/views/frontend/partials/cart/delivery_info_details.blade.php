    ...
    <div class="{{ $col_val }}">
        <ul class="list-group list-group-flush mb-3">
            @foreach ($products as $key => $cartItem)
                @php
                    $product = get_single_product($cartItem);
                    $productName = $product ? $product->getTranslation('name') : 'Unknown Product';
                    $productImage = $product
                        ? get_image($product->thumbnail)
                        : static_asset('assets/img/placeholder.jpg');
                    $variationText = !empty($product_variation[$key]) ? $product_variation[$key] : null;
                    $value = $cartItem->price ?? 0; // Assuming price comes from $cartItem
                @endphp
                <li class="list-group-item pl-0 py-3 border-0">
                    <div class="d-flex align-items-center">
                        <span class="mr-2 mr-md-3">
                            <img src="{{ $productImage }}" class="img-fit size-60px" alt="{{ $productName }}"
                                onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder.jpg') }}';">
                        </span>
                        <span class="d-flex flex-row justify-content-between">
                            <span class="fs-14 fw-400 text-dark">
                                <span class="text-truncate-2">{{ $productName }}</span>
                                @if ($variationText)
                                    <span class="fs-12 text-secondary">{{ translate('Variation') }}:
                                        {{ $variationText }}</span>
                                @endif
                            </span>
                            <span class="ml-auto" style="margin-left: 50px !important;">
                                {{ home_discounted_price_cart($product ?? null, $value) }}
                            </span>
                        </span>
                    </div>
                </li>
        ...
        <div class="col-md-12 mb-2">
            ...
            <div class="row gutters-16">
                ...
                @if (get_setting('shipping_type') != 'carrier_wise_shipping')
                    <div class="col-6">
                        <label class="aiz-megabox d-block bg-white mb-0">
                            <input type="radio" name="shipping_type_{{ $owner_id }}" value="home_delivery"
                                onchange="show_pickup_point(this, {{ $owner_id }})"
                                data-target=".pickup_point_id_{{ $owner_id }}" checked required>
                ...
                @else
                    <div class="col-6">
                        <label class="aiz-megabox d-block bg-white mb-0">
                            <input type="radio" name="shipping_type_{{ $owner_id }}" value="carrier"
                                data-owner="{{ $owner_id }}"
                                onchange="show_pickup_point(this, {{ $owner_id }})"
                                data-target=".pickup_point_id_{{ $owner_id }}" checked required>
                            <span class="d-flex aiz-megabox-elem rounded-0" style="padding: 0.75rem 1.2rem;">
                ...
                @if ($pickup_point_list)
                    <div class="col-6">
                        <label class="aiz-megabox d-block bg-white mb-0">
                            <input type="radio" name="shipping_type_{{ $owner_id }}" value="pickup_point"
                                data-owner="{{ $owner_id }}"
                                onchange="show_pickup_point(this, {{ $owner_id }})"
                                data-target=".pickup_point_id_{{ $owner_id }}" required>
                            <span class="d-flex aiz-megabox-elem rounded-0" style="padding: 0.75rem 1.2rem;">
                                <span class="aiz-rounded-check flex-shrink-0 mt-1"></span>
            ...
            @if ($pickup_point_list)
                <div class="mt-3 pickup_point_id_{{ $owner_id }} d-none">
                    <select class="form-control aiz-selectpicker rounded-0" name="pickup_point_id_{{ $owner_id }}"
            ...
            @if (get_setting('shipping_type') == 'carrier_wise_shipping')
                <div class="row pt-3 carrier_id_{{ $owner_id }}">
                    @foreach ($carrier_list as $carrier_key => $carrier)
                        <div class="col-md-12 mb-2">
                            <label class="aiz-megabox d-block bg-white mb-0">
                                <input type="radio" name="carrier_id_{{ $owner_id }}"
                                ...
                                <span class="d-flex flex-wrap p-3 aiz-megabox-elem rounded-0">
                                    ...
                                    <span
                                        class="flex-grow-1 pl-4 pl-sm-3 fw-600 mt-2 mt-sm-0 text-sm-right">{{ single_price(carrier_base_price($carts, $carrier->id, $owner_id, $shipping_info)) }}</span>
                                </span>
                            </label>
                        </div>
                    @endforeach
