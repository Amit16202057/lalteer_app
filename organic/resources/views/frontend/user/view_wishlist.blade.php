...
@section('panel_content')
    ...
    <div>
        @if (count($wishlists) > 0)
            <table class="table">
                <thead style="background-color: #D9EDC4;">
                    <tr>
                        ...
                        <tr id="wishlist_{{ $wishlist->id }}">
                            ...
                            <td class="d-flex flex-row align-items-center">
                                <a href="{{ route('product', $wishlist->product->slug) }}"
                                    ...
                                    <img src="{{ uploaded_asset($wishlist->product->thumbnail_img) }}" class="img-thumbnail"
                                        style="max-width: 80px; height: auto;"
                                        alt="{{ $wishlist->product->getTranslation('name') }}">
                                </a>
                                <span class="d-flex flex-column" style="margin-left: 10px;">
                                    <span class="fs-15 text-success fw-600">
                                        {{ $wishlist->product->getTranslation('name') }}
                                    </span>
                                    <span style="margin-top: 5px;">
                                        <a class="bg-danger rounded-1 text-white p-1 cursor-pointer"
                            ...
                            <td>
                                @if ($wishlist->product->stock > 0)
                                    <span class="text-success fw-700">In Stock</span>
                                @else
                                    <span class="text-success opacity-60 fw-700">{{ translate('Out of Stock') }}</span>
                                @endif
                            </td>
                            <!-- Price -->
                            <td>
                                <span class="text-primary">{{ home_discounted_base_price($wishlist->product) }}</span>
                    ...
                    <div class="position-relative h-140px h-md-200px img-fit overflow-hidden mb-3">
                        ...
                        <a href="{{ route('product', $wishlist->product->slug) }}" class="d-block h-100">
                            <img src="{{ uploaded_asset($wishlist->product->thumbnail_img) }}"
                                class="lazyload mx-auto img-fit" title="{{ $wishlist->product->getTranslation('name') }}">
                        </a>
